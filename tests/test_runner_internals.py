...
def test_tool_loop_timeout_marks_turn_timeout(tools_scenarios_path: Path, fake_llm) -> None:
...
def test_tool_loop_unreachable_marks_run_unreachable(tools_scenarios_path: Path, fake_llm) -> None:
    scenario = load_scenarios(tools_scenarios_path).scenarios[0]
...
def test_unreachable_on_text_round_sets_error_flag(small_scenarios_path: Path, fake_llm) -> None:
...
def test_text_round_agent_error_marks_turn(small_scenarios_path: Path, fake_llm) -> None:
    scenario = load_scenarios(small_scenarios_path).scenarios[0]
    fake_llm.queue_compact([1, 1])
...
def test_may_diverge_true_path_populates_standard_checks(fake_llm) -> None:
    raw = {
        "version": "2.0",
        "scenarios": [
            {
    ...
    res = runner.run_scenario(scenario, runs=1)
...
def test_may_diverge_true_eval_error_marks_turn(fake_llm) -> None:
    raw = {
        "version": "2.0",
        "scenarios": [
            {
...
def test_custom_check_eval_error_on_exact_match_path(fake_llm) -> None:
    raw = {
        "version": "2.0",
        "scenarios": [
            {
...
def test_no_outcome_checks_short_circuits_outcome_eval(fake_llm) -> None:
    raw = {
        "version": "2.0",
        "scenarios": [
            {
...
def test_text_round_timeout_marks_turn_timeout(small_scenarios_path: Path, fake_llm) -> None:
    scenario = load_scenarios(small_scenarios_path).scenarios[0]
...
def test_tool_loop_agent_error_branch(tools_scenarios_path: Path, fake_llm) -> None:
    scenario = load_scenarios(tools_scenarios_path).scenarios[0]
...
def test_unexpected_tool_in_loop_is_recorded(fake_llm) -> None:
    raw = {
        "version": "2.0",
        "scenarios": [
            {
...
def test_outcome_eval_error_branch(small_scenarios_path: Path, fake_llm) -> None:
    scenario = load_scenarios(small_scenarios_path).scenarios[0]
